...
def convert_mp3_to_wav_robust(file_info):
    ...
    # Try multiple conversion strategies for MP3 to WAV
    conversion_attempts = [
        # Attempt 1: High quality PCM WAV (16-bit, 44.1kHz)
        [ffmpeg_path, "-y", "-i", input_path, "-acodec", "pcm_s16le", "-ar", "44100", "-ac", "2", output_path],
        
        # Attempt 2: Standard quality PCM WAV (16-bit, same as source sample rate)
        [ffmpeg_path, "-y", "-i", input_path, "-acodec", "pcm_s16le", output_path],
        
        # Attempt 3: 24-bit WAV (high quality)
        [ffmpeg_path, "-y", "-i", input_path, "-acodec", "pcm_s24le", "-ar", "44100", output_path],
        
        # Attempt 4: Basic WAV conversion (let ffmpeg decide)
        [ffmpeg_path, "-y", "-i", input_path, "-f", "wav", output_path],
        
        # Attempt 5: Most basic conversion
...
def main():
    ...
    INPUT_DIRECTORY = r"C:\RESEARCH 2\Paper 1\archive (1)\Dataset\mp3 converted back"  # Your MP3 files
    OUTPUT_DIRECTORY = r"C:\RESEARCH 2\Paper 1\archive (1)\Dataset\wav from mp3"  # Output WAVs
