...
# --------- CONFIG --------
stego_path = r"/input/stego-file/Dataset/stego/"
sr = 22050
TARGET_VALID_FILES = 10400
INVALID_LOG = "invalid_stego_files.txt"
OUTPUT_H5 = "stego_features_MDCT.h5"
# --------------------------
...
def MDCT(x, N):
    """Simplified MDCT implementation with sine window"""
    x = np.pad(x, (N // 2, N // 2), mode='constant')
    num_blocks = len(x) // N - 1
    result = []

    for i in range(num_blocks):
        start = i * N
        end = start + 2 * (N // 2)
        window = np.sin(np.pi / (2 * N) * (np.arange(2 * (N // 2)) + 0.5))
        segment = x[start:end] * window
        coeffs = dct(segment, type=2, norm='ortho')[:N // 2]
        result.append(coeffs)

    return np.array(result)

def MDCT_Feature_Extractor(file_path):
    try:
        y, _ = librosa.load(file_path, sr=sr, duration=5.0)
        y_harmonic, _ = librosa.effects.hpss(y)
        mdct_feat = MDCT(y_harmonic, N=512)  # Try N=512 or 1024 for different resolutions
        return mdct_feat
    except Exception as e:
...
def extract_valid_features(folder_path, label_desc, target_valid_files=TARGET_VALID_FILES):
    ...
    for file in tqdm(all_files, desc=label_desc):
        ...
        feat = MDCT_Feature_Extractor(path)
        if feat is not None:
...
stego_features = extract_valid_features(stego_path, "Stego Files", target_valid_files=TARGET_VALID_FILES)
pad_and_save(stego_features, OUTPUT_H5)
